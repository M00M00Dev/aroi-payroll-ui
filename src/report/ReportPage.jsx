// Version: 2603271145-REPORT-FINAL
import React, { useState, useEffect } from 'react';
import { ChevronLeft, Download, Copy, Store, Wallet, Users, Printer, Loader2, CheckCircle2, AlertCircle } from 'lucide-react';
import { generateABA } from './abaGenerator'; // Import our new utility
...
const ReportPage = () => {
    ...
    const copyDirectSummary = (staffList) => {
        const text = staffList.map(s => `${s.name}: $${s.total}`).join('\n');
        navigator.clipboard.writeText(text);
        alert("Direct Summary Copied!");
    };
        ...
        <div className="min-h-screen bg-[#F8FAFC] p-6 font-sans antialiased text-slate-800">
            <div className="max-w-6xl mx-auto flex justify-between items-end mb-8">
                <div>
                    ...
                    <p className="text-slate-500 font-medium text-xs uppercase tracking-widest">Fortnight Start: {startDate}</p>
                </div>
                <button onClick={() => window.print()} className="bg-white border border-slate-200 p-2.5 rounded-xl hover:bg-slate-50 shadow-sm transition-all text-slate-400 hover:text-slate-900"><Printer size={18}/></button>
            </div>
            ...
            <div className="max-w-6xl mx-auto space-y-12">
                {Object.entries(reportData || {}).map(([shopName, categories]) => (
                    <div key={shopName} className="space-y-6">
                        ...
                        {/* TFN SECTION */}
                        {categories.TFN?.length > 0 && (
                            <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
                                <div className="bg-slate-900 px-4 py-2 flex justify-between items-center text-white">
                                    <div className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-widest">
                                        <Users size={14} className="text-emerald-400" /> TFN Payroll (ABA)
                                    </div>
                                    <button 
                                        onClick={() => generateABA(shopName, categories.TFN)}
                                        className="bg-emerald-600 hover:bg-emerald-500 text-[9px] font-black px-3 py-1.5 rounded-lg transition-all flex items-center gap-1 uppercase"
                                    >
                                        <Download size={12}/> Download ABA
                                    </button>
                                </div>
                                <table className="w-full text-left text-[11px]">
                                    <thead className="bg-slate-50 border-b border-slate-100 text-slate-400 font-bold uppercase">
                                        <tr>
                                            ...
                                            <th className="p-4 text-center">Wk / We Rates</th>
                                            <th className="p-4 text-center">Hrs (Wk/We)</th>
                                    ...
                                    <tbody className="divide-y divide-slate-50">
                                        {categories.TFN.map((emp, i) => (
                                            <tr key={i} className="hover:bg-slate-50/50 transition-colors">
                                                ...
                                                <td className="p-4 text-center text-slate-400 font-mono">${emp.rate_wk} / ${emp.rate_we}</td>
                                                <td className="p-4 text-center font-bold text-slate-700">{emp.hrs_wk} / {emp.hrs_we}</td>
                                                <td className="p-4 text-center">
                                                    {emp.bank?.length > 5 ? <CheckCircle2 size={14} className="text-emerald-500 mx-auto" /> : <AlertCircle size={14} className="text-red-400 mx-auto" />}
                                                </td>
                        ...
                        {/* DIRECT SECTION */}
                        {categories.Direct?.length > 0 && (
                            <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
                                <div className="bg-blue-900 px-4 py-2 flex justify-between items-center text-white">
                                    <div className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-widest">
                                        <Wallet size={14} className="text-blue-300" /> Direct (Manual)
                                    </div>
                                    <button 
                                        onClick={() => copyDirectSummary(categories.Direct)}
                                        className="bg-blue-700 hover:bg-blue-600 text-[9px] font-black px-3 py-1.5 rounded-lg transition-all flex items-center gap-1 uppercase"
                                    >
                                ...
                                <table className="w-full text-left text-[11px]">
                                    <thead className="bg-slate-50 border-b border-slate-100 text-slate-400 font-bold uppercase">
                                        <tr>
                                            <th className="p-4">Name</th>
                                            <th className="p-4 text-center">Total Hrs</th>
