// Version: 260326-FULL-INTEGRATION (Grid + Report + Today + J/K Navigation)
...
const App = () => {
  ...
  const ReportOverlay = () => {
    ...
    return (
      <div className="fixed inset-0 z-[500] bg-white overflow-y-auto font-sans text-slate-900 p-8 animate-in fade-in zoom-in duration-200">
          ...
          {shops.map(shop => (
            <div key={shop} className="mb-16">
              <div className="flex justify-between items-end mb-6">
                <h2 className="text-2xl font-black uppercase text-slate-800 flex items-center gap-3"><div className="w-2 h-8 bg-emerald-500 rounded-full"/> {shop}</h2>
                      ...
                      return (
                        <tr key={emp.id} className={`hover:bg-slate-50/50 transition-colors ${paidStaff.has(emp.id) ? 'bg-emerald-50/30' : ''}`}>
                          <td className="p-4 text-center">
                            <button onClick={() => setPaidStaff(prev => { const next = new Set(prev); next.has(emp.id) ? next.delete(emp.id) : next.add(emp.id); return next; })}
                                className={`p-1.5 rounded-lg transition-all ${paidStaff.has(emp.id) ? 'text-emerald-600 bg-emerald-100' : 'text-slate-200 bg-slate-50 border border-slate-100'}`}><CheckCircle2 size={18} /></button>
                          </td>
      ...
      <div className="max-w-[1900px] mx-auto">
        ...
        <div className="flex justify-between items-center mb-4 bg-white p-3 rounded-2xl shadow-sm border border-slate-200">
          ...
          <button onClick={async () => {
            ...
            {isSyncing ? <Loader2 size={14} className="animate-spin" /> : <Save size={14} />} <span>{isSyncing ? 'Save Sync' : 'Save Sync'}</span>
          </button>
        </div>
                    ...
                    <tr key={emp.id} className={`group divide-x divide-slate-50 transition-all ${emp.approved ? 'bg-amber-50/40' : isHidden ? 'row-hidden' : 'hover:bg-slate-50/50'}`}>
                      ...
                      {!isHidden ? emp.daily?.map((day, idx) => (
                        <React.Fragment key={idx}>
                          ...
                          }} className="w-full bg-transparent text-right text-[11px] font-bold font-mono text-emerald-700" /></td>}
                        </React.Fragment>
                      )) : <td colSpan={20} className="bg-slate-50/50" />}
                      {!isHidden && (
                        <>
                          <td className="p-2 text-center font-bold text-slate-400">{b.weekday.toFixed(1)}</td>
                          <td className="p-2 text-center font-bold text-orange-600">{b.weekend.toFixed(1)}</td>
                          <td className="p-2 text-center font-black text-[13px] bg-emerald-50 text-emerald-900">{(b.weekday + b.weekend).toFixed(1)}</td>
                          <td className="p-2 text-center"><button onClick={() => toggleApprove(emp.id)} className={`w-full py-1.5 rounded flex items-center justify-center gap-1.5 font-black uppercase text-[9px] ${emp.approved ? 'bg-amber-600 text-white shadow-lg' : 'bg-white border border-slate-200 text-slate-400'}`}>
