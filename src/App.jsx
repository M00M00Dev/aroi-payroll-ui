// Version: 260326-STABLE-RESTORED
...
const App = () => {
  const APP_VERSION = "260326-STABLE";
            ...
            <table className="w-full text-left border-collapse table-fixed min-w-[1800px]">
              ...
              <tbody className="divide-y divide-slate-100">
                {data.map((emp) => {
                  ...
                  return (
                    <tr key={emp.id} className={`group divide-x divide-slate-50 transition-all ${emp.approved ? 'bg-amber-50/40' : 'hover:bg-slate-50/50'}`}>
                      <td className="p-3 sticky left-0 z-20 bg-white border-r border-slate-200 group-hover:bg-slate-50 overflow-hidden">
                        <div className="flex items-center gap-3">
                          ...
                          <span className={`font-bold uppercase tracking-tighter truncate text-[11px] ${emp.approved ? 'text-amber-700' : 'text-slate-700'}`}>{emp.name}</span>
                        </div>
                      </td>
                      {emp.daily?.map((day, idx) => (
                        <React.Fragment key={idx}>
                          <td className={`p-1 ${idx % 7 >= 5 ? 'bg-orange-50/30' : ''}`}>
                            <div className={`p-1 rounded-lg border flex flex-col items-center transition-colors ${emp.approved ? 'border-amber-100 bg-amber-50/20' : day.s > 0 ? 'bg-emerald-50 border-emerald-100' : 'border-transparent'}`}>
